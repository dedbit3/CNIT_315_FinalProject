...
/* end program if only one argument is inputted... */
...
/* getting a handle to the process... */
	...
	STATUS = heckingOpen(&hProcess, PROCESS_ALL_ACCESS, &OA, &CID);
...
/* allocating buffer to process memory... */
...
CLEANUP:
	...
	if (hThread) {
		heckingClose(hThread);
		info("closed handle on thread");
	}

	if (hProcess) {
		heckingClose(hProcess);
